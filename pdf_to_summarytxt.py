...
************************************
python openai_arrange.py --input 対象のテキストファイル名（.txtなしで） --api_key 自分のopenAIのAPIキー
************************************
契約しているChatGPTのモデルが違う場合は--model で指定できますが動作未確認. 詳細はOpenAIの公式ドキュメントをご確認ください.
要約速度は契約のプランによって変化します.
PDFを要約したい場合は事前にextract_doc_info_toCSV.pyでPDFからテキストファイルに変更してください.
'''
